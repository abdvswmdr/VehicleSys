...
- [X] Backend (Qt/C++ Controllers)
    - [x] Vehicle data (telemetry, gauges)
    - [X] HVAC (dual-zone, climate, 50-90°F)
    - [X] Media control (Qt Multimedia, volume sync)
    - [X] CAN bus controller (vcan0 interface)             
    - [X] System state (connectivity/[un]lock indicators)
    - [ ] Location search (async, QtLocation)     
- [X] UI Frontend (QML)
    - [X] Dual-screen (responsive)         
    - [X] <s>OpenStreetMap</s> MapboxGL 
...
#+HTML: <input type="checkbox" checked disabled> Completed item
#+HTML: <input type="checkbox" disabled> Incomplete item
